...
class RNNPredictor_sumEmb(PredictorBase):
    ...
    def __init__(self,
        ...
        self.ILMA = ILMA
        self.sum_embed = nn.Embedding(voca_size-1, output_size)
        self.out_linear = nn.Linear(output_size, voca_size-1)
    ...
    def forward(
        ...
        ILM_out = self.out_linear(out)
        if self.ILMA:
            out = None
        else:
            ILM_out_pro = torch.nn.functional.softmax(ILM_out, dim=-1)
            out = (ILM_out_pro.unsqueeze(-1) * self.sum_embed.weight.unsqueeze(0).unsqueeze(0)).sum(2)
