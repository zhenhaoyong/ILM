...
train_set=AISHELL1
# train_set=AISHELL2
train_config=conf/conformer_u2pp_rnnt_ILMT.yaml
# train_config=conf/conformer_u2pp_rnnt_ILMA.yaml
cmvn=true
dir=exp/conformer_rnnt_ILMT_plus
# checkpoint=$dir/init.pt
checkpoint=

# use average_checkpoint will get better result
average_checkpoint=false
decode_checkpoint=$dir/avg_19.pt
average_num=19
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  for ((i = 0; i < $num_gpus; ++i)); do
  {
    ...
    python wenet/bin/train.py --gpu $gpu_id \
      ...
      --cv_data data/AISHELL_dev/data.list \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  {
    test_dir=$dir/test_${mode}/$decode_checkpoint/
    mkdir -p $test_dir
    python wenet/bin/recognize.py --gpu 1 \
      --mode $mode \
      --config $dir/train.yaml \
      --data_type $data_type \
      --test_data data/$test_sets/data.list \
    ...
    python tools/compute-wer.py --char=1 --v=1 \
      data/$test_sets/text $test_dir/text > $test_dir/wer
  } &
  done
  wait
fi
